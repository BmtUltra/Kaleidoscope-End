...
public class KETeacups {
    ...
    public static void init() {
        TeacupRegistry registry = new TeacupRegistry() {
            ...
            public ResourceLocation registerTeacupData(String name, TeacupData data) {
                ResourceLocation id = ResourceLocation.fromNamespaceAndPath(KaleidoscopeEnd.MOD_ID, name);
        ...
        VOID_TEA = registry.registerTeacupData("void_tea",
                TeacupRegistry.TeacupData.create(4)
                        .addEffect(() -> new MobEffectInstance(KEEffects.VOID_EROSION, 3 * 60 * 20))
        );

        // 末影薄荷茶
        ENDER_MINT_TEA = registry.registerTeacupData("ender_mint_tea",
                TeacupRegistry.TeacupData.create(4)
                        .addEffect(() -> new MobEffectInstance(KEEffects.MINT, 5 * 60 * 20))
        );

        // 紫颂花茶
        CHORUS_FLOWER_TEA = registry.registerTeacupData("chorus_flower_tea",
                TeacupRegistry.TeacupData.create(4)
                        .addEffect(() -> new MobEffectInstance(KEEffects.DREAM, 3 * 60 * 20))
        );
    ...
    public static ResourceLocation id(String name) {
        return ResourceLocation.fromNamespaceAndPath(KaleidoscopeEnd.MOD_ID, name);
    }
}